...
// Estatus segun documentos (misma logica que Gastos Diarios)
function calcularEstatus(orden_compra: string | null, factura: string | null): string {
  if (factura && factura.trim() !== '') return 'Completado'
  if (orden_compra && orden_compra.trim() !== '') return 'Pendiente Factura'
  return 'Pendiente OC'
}

export default function CargaGastosView() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const stats = {
        ...
        piezas: preview.filter(g => g.clasificacion === 'PIEZAS DE BODEGA').length,
        otros: preview.filter(g => g.clasificacion !== 'CAJA CHICA' && g.clasificacion !== 'PIEZAS DE BODEGA' && g.clasificacion !== 'N/A').length,
  ...
  const confirmarCarga = async () => {
    ...
    try {
      const registrosInsertar = previewData
        ...
        .map(p => {
          const oc = p.orden_compra === 'N/A' ? null : p.orden_compra
          const fac = p.factura === 'N/A' ? null : p.factura
          return {
            ...
            estatus: calcularEstatus(oc, fac),
            es_cierre: false,
          }
        })
      ...
      setMessage({ type: 'success', text: `¡Carga completada! ${registrosInsertar.length} registros agregados a gastos diarios.` })
    } catch (err: any) {
      setMessage({ type: 'error', text: 'Error en carga: ' + err.message })
    } finally {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Carga de Gastos</h2>
        <p className="text-sm text-slate-500 mt-1">Sube el archivo de CAJA CHICA y PIEZAS DE BODEGA para agregar gastos (sin borrar nada)</p>
      </div>
      ...
      <div className="card-solid">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-bold text-slate-800">Subir archivo de Caja Chica / Piezas de Bodega</h3>
            <p className="text-sm text-slate-500">Archivo Excel con los gastos de Caja Chica y Piezas de Bodega de {MESES[mes-1]} {año}</p>
          </div>
        </div>
      ...
      {showPreview && previewData.length > 0 && (
        <div className="card-solid space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-slate-800">Previsualización de la Carga</h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            ...
            <div className="bg-slate-50 p-3 rounded-xl text-center border border-slate-100">
              <p className="text-2xl font-bold text-blue-600">{resumenStats.piezas}</p>
              <p className="text-xs text-slate-500 font-medium">Piezas Bodega</p>
            </div>
            <div className="bg-slate-50 p-3 rounded-xl text-center border border-slate-100">
                  ...
                  <tr key={idx} className={g.tienda_id ? '' : 'bg-red-50/50'}>
                    ...
                    <td>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        ...
                        g.clasificacion === 'PIEZAS DE BODEGA' ? 'bg-blue-100 text-blue-700' :
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-slate-100">
            <button onClick={confirmarCarga} disabled={confirmando || previewData.filter(p => p.tienda_id !== null).length === 0} className="btn-primary flex items-center gap-2 disabled:opacity-50">
              {confirmando ? 'Procesando...' : <><Check className="w-4 h-4" />Confirmar Carga</>}
            </button>
            <button onClick={() => { setPreviewData([]); setShowPreview(false); }} className="btn-secondary flex items-center gap-2">
              <X className="w-4 h-4" />Cancelar
            </button>
            <p className="text-sm text-slate-500">
              Se insertarán {previewData.filter(p => p.tienda_id !== null).length} registros nuevos · No se borra nada
            </p>
          </div>
        </div>
      )}
